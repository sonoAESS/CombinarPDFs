...
def fondo_redondeado() -> Image.Image:
...
def hoja(
    ...
    relleno: tuple[int, int, int],
    doblez_color: tuple[int, int, int],
) -> None:
...
def insignia(img: Image.Image) -> None:
    """Círculo azul con '+' blanco en la esquina inferior derecha."""
    d = ImageDraw.Draw(img)
    cx, cy, r = int(S * 0.70), int(S * 0.72), int(S * 0.155)
    d.ellipse(
        [cx - r - 10, cy - r - 10, cx + r + 10, cy + r + 10], fill=COLOR_FONDO_BOTTOM
    )
    d.ellipse(
        [cx - r, cy - r, cx + r, cy + r],
        fill=COLOR_ACENTO,
        outline=COLOR_ACENTO_CLARO,
        width=max(4, S // 170),
    )
    grosor = max(10, int(r * 0.28))
    brazo = int(r * 0.62)
    d.rounded_rectangle(
        [cx - brazo, cy - grosor // 2, cx + brazo, cy + grosor // 2],
        radius=grosor // 2,
        fill=(255, 255, 255),
    )
    d.rounded_rectangle(
        [cx - grosor // 2, cy - brazo, cx + grosor // 2, cy + brazo],
        radius=grosor // 2,
        fill=(255, 255, 255),
    )
...
def generar() -> None:
    ...
    hoja(
        img,
        d,
        (int(S * 0.20), int(S * 0.14), int(S * 0.58), int(S * 0.66)),
        COLOR_HOJA_TRASERA,
        COLOR_DOBLEZ,
    )
    hoja(
        img,
        d,
        (int(S * 0.34), int(S * 0.26), int(S * 0.76), int(S * 0.82)),
        COLOR_HOJA,
        COLOR_DOBLEZ,
    )
